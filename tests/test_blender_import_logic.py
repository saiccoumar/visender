...
def test_load_config_reads_the_json_written_by_visender(bi, tmp_path):
...
def test_check_bundle_version_accepts_the_current_format(bi):
    bi.check_bundle_version({"format": "visender", "version": 1}, Path("b"))
...
def test_check_bundle_version_rejects_a_newer_bundle(bi):
    with pytest.raises(SystemExit) as exc:
        bi.check_bundle_version({"format": "visender", "version": 99}, Path("b"))
    assert "Update visender" in str(exc.value)
