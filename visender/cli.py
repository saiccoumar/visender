...
try:
    ...
    if exc.name != "yaml":
        raise
    raise SystemExit(
        "visender needs pyyaml to read a config file: pip install 'visender[cli]' "
...
def cmd_render(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="visender render")
    ...
    print(f"[visender] {' '.join(cmd)}", flush=True)
    try:
...
def cmd_list_nodes(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="visender list-nodes")
